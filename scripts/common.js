...
exports.getEnvironmentVariables = function getEnvironmentVariables() {
  ...
  try {
    ...
    const workspace_raw = shell.exec(
      "twilio api:taskrouter:v1:workspaces:list",
      { silent: true }
    );
    if (workspace_raw.length > 0) {
      result.taskrouter_workspace_sid = workspace_raw
        .grep(FLEX_WORKSPACE_NAME)
        .stdout.split(" ")[0];
    } else {
    ...
    const sync_raw = shell.exec("twilio api:sync:v1:services:list", {
      silent: true,
    });
    result.sync_sid =
      sync_raw.length > 0
        ? sync_raw.grep(SYNC_SERVICE_NAME).stdout.split(" ")[0]
        : console.log("Sync Service Not Found");

    const chat_raw = shell.exec("twilio api:chat:v2:services:list", {
      silent: true,
    });
    result.chat_sid =
      chat_raw.length > 0
        ? chat_raw.grep(CHAT_SERVICE_NAME).stdout.split(" ")[0]
        : console.log("Chat Service Not Found");

    var workflows = shell.exec(
      `twilio api:taskrouter:v1:workspaces:workflows:list --workspace-sid=${result.taskrouter_workspace_sid}`,
      { silent: true }
    );
    if (workflows.length > 0) {
      result.everyoneWorkflow = workflows
        .grep(ANYONE_WORKFLOW_NAME)
        .stdout.split(" ")[0]
        .trim();
      result.chatTransferWorkFlow = workflows
        .grep(CHAT_WORKFLOW_NAME)
        .stdout.split(" ")[0]
        .trim();
      result.callbackWorkflow = workflows
        .grep(CALLBACK_WORKFLOW_NAME)
        .stdout.split(" ")[0]
        .trim();
      result.internalCallWorkflow = workflows
        .grep(INTERNAL_CALL_WORKFLOW_NAME)
        .stdout.split(" ")[0]
        .trim();
    } else {
...
exports.getServerlessServices = function getServerlessServices() {
  ...
  const serverless_services_raw = shell.exec(
    "twilio api:serverless:v1:services:list",
    { silent: true }
  );
  if (serverless_services_raw.length > 0) {
    result.serviceFunctionsSid = serverless_services_raw
      .grep(SERVERLESS_FUNCTIONS_SERVICE_NAME)
      .stdout.split(" ")[0];
    result.serviceFunctionsDomain = shell
      .exec(
        `twilio api:serverless:v1:services:environments:list --service-sid=${result.serviceFunctionsSid}`,
        { silent: true }
      )
      .grep(SERVERLESS_FUNCTIONS_SERVICE_NAME)
      .stdout.split(" ")[4];

    result.scheduleFunctionsSid = serverless_services_raw
      .grep(SCHEDULE_MANAGER_SERVICE_NAME)
      .stdout.split(" ")[0];
    const scheduledFunctionsDomain_raw = shell.exec(
      `twilio api:serverless:v1:services:environments:list --service-sid=${result.scheduleFunctionsSid}`,
      { silent: true }
    );
    result.scheduledFunctionsDomain =
      scheduledFunctionsDomain_raw.length > 0
        ? scheduledFunctionsDomain_raw
            .grep(SCHEDULE_MANAGER_SERVICE_NAME)
            .stdout.split(" ")[4]
        : console.log("Scheduled Functions Domain not found");
  } else {
...
exports.getActiveTwilioProfile = async function getActiveTwilioProfile() {
  ...
  try {
    result.profile = await shell
      .exec("twilio profiles:list", { silent: true })
      .grep("true").stdout;
...
exports.installNPMServerlessFunctions =
  function installNPMServerlessFunctions() {
...
exports.installNPMServerlessSchmgrFunctions =
  function installNPMServerlessSchmgrFunctions() {
    if (shell.test("-d", scheduleManagerServerlessDir)) {
      console.log(
        "Installing npm dependencies for serverless schedule manager.."
      );
...
exports.generateServerlessFunctionsEnv =
  function generateServerlessFunctionsEnv(
    ...
    try {
      ...
      if (process.env.TWILIO_API_KEY && !api_key)
      ...
      if (shell.test("-e", serverlessEnv)) {
        ...
        if (callbackWorkflow || everyoneWorkflow) {
          ...
          shell.sed(
            "-i",
            /<YOUR_FLEX_CALLBACK_WORKFLOW_SID>/g,
            `${workflow}`,
            serverlessEnv
          );
        }
        if (internalCallWorkflow) {
          shell.sed(
            "-i",
            /<YOUR_FLEX_INTERNAL_CALL_WORKFLOW_SID>/g,
            `${internalCallWorkflow}`,
            serverlessEnv
          );
        }
        if (chatTransferWorkFlow) {
          shell.sed(
            "-i",
            /<YOUR_FLEX_CHAT_TRANSFER_WORKFLOW_SID>/g,
            `${chatTransferWorkFlow}`,
            serverlessEnv
          );
        }
        ...
        console.warn(
          "Unable to configure serverless environment file, it will need to be done manually"
        );
      }
    } catch (error) {
      console.error(error);
      console.log(
        `Error attempting to generate serverless environment file ${serverlessEnv}`
      );
    }
    ...
    try {
      ...
      console.error(error);
      console.log(
        `Error attempting to generate schedule manager environment file ${serverlessEnv}`
      );
    }
  };

exports.generateFlexConfigEnv = function generateFlexConfigEnv(
  context,
  flexConfigEnv
) {
  const flexConfigEnvExample = `./${flexConfigDir}/.env.example`;
  const { account_sid, api_key, api_secret, auth_token } = context;
  ...
  try {
    ...
    if (shell.test("-e", flexConfigEnv)) {
      ...
      console.warn(
        "Unable to configure flex config environment file, it will need to be done manually"
      );
    }
  } catch (error) {
    console.error(error);
    console.log(
      `Error attempting to generate flex config environment file ${flexConfigEnv}`
    );
  }
};

exports.populateFlexConfigPlaceholders =
  function populateFlexConfigPlaceholders(context, environment) {
    const configFile = `./${flexConfigDir}/ui_attributes.${environment}.json`;

    var { scheduledFunctionsDomain, serviceFunctionsDomain } = context;

    if (shell.test("-e", configFile)) {
      if (serviceFunctionsDomain) {
        shell.sed(
          "-i",
          /<PLACEHOLDER_SERVERLESS_DOMAIN>/g,
      ...
      if (scheduledFunctionsDomain) {
        shell.sed(
          "-i",
          /<PLACEHOLDER_SCHEDULE_MANAGER_DOMAIN>/g,
  ...
  if (pluginDir && pluginDir != "") {
    try {
      if (!shell.test("-e", pluginAppConfig)) {
        ...
        appConfigFileData = appConfigFileData.replace(
          "features: { }",
          `features: ${JSON5.stringify(
            flexConfigJsonData.custom_data.features,
            null,
            2
          )}`
        );
      ...
      console.error(error);
      console.log(
        `Error attempting to generate appConfig file ${pluginAppConfig}`
      );
    }
  }
