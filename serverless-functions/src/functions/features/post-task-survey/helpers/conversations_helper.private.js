...
exports.createOutboundCustomerConversation = async (client, workspaceSid, to, from) => {
  ...
  try {
    const participant = await client.conversations.v1.conversations(conversation.sid).participants.create({
    ...
    if (error.code === 50416) {
      ...
      const existingConversation = await client.conversations.v1.conversations(existingConversationSid).fetch();

      console.log('Active conversation for this messagingBinding participant', existingConversation);

      const agentIdentity = await agentIdentityFromConversation(client, existingConversationSid);

      const agentName = await agentNameFromIdentity(client, workspaceSid, agentIdentity);

      const { taskExists, taskDirection } =
        (await taskDetailsFromConversationSid(client, workspaceSid, existingConversationSid)) || {};
...
exports.setKnownAgentRoutingOnConversationAttributes = async (client, conversation, WorkerSid) => {
...
exports.setStudioWebhookOnConversation = async (client, conversation, InboundStudioFlow) => {
...
exports.addAgentMessageToConversation = async (client, conversation, WorkerFriendlyName, Body, ContentTemplateId) => {
  if (ContentTemplateId) {
    const contentMessage = await client.conversations.v1
      .conversations(conversation.sid)
      .messages.create({ author: WorkerFriendlyName, contentSid: ContentTemplateId });
  } else {
    const simpleMessage = await client.conversations.v1
      .conversations(conversation.sid)
